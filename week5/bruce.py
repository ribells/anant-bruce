...
class Ballistic:
    ...
    def __init__(self, start_ll, end_ll,
        ...
        self.v0 = math.sqrt(self.range_m*self.g / math.sin(2*self.theta))                #total velocity
        self.vx0, self.vz0 = self.v0*math.cos(self.theta), self.v0*math.sin(self.theta)  #velocities in x and z components

        self.states = None     # list of [x,z,vx,vz]
        self.pts    = None     # list of local points (scaled), origin at v_start

    def _rk4_step(self, state):  # perform the next timestep
        # state = [x, z, vx, vz]; a_x = 0, a_z = -g
        x, z, vx, vz = state
    ...
    def simulate(self):
        ...
        states = []
        for _ in range(self.n_steps):  # simulate ballistic movement at each timestep
            states.append(s.copy())    # create a state array entry by copying last state array
            s = self._rk4_step(s)      # calculate the state array after next time step
        self.states = states           # update the ballistic object

        pts = []                       # pts are where ballistic is at each timestep
        for s in states:               # iterate all states to generate points for each location
            x, z, _, _ = s             # _ means not used
            world = self.v_start + self.ex*x + self.ez*z  # update world location by applying movement
...
B = Ballistic(PVD, DC, launch_angle_deg=LAUNCH_ANGLE_DEG, g=G,
              n_steps=N_STEPS, dt=DT, scale=SCALE, name="Traj")
pts = B.simulate()  # list of local (scaled) points
mat = bpy.data.materials.new("TrajectoryMat")
...
class Missile:
    def __init__(self, radius, depth, location, material=None):
        ...
        bpy.ops.mesh.primitive_cone_add(radius1=radius*1.0, depth=2.0, location=(location[0], location[1], nose_z))
...
mat = bpy.data.materials.new("MissileMat")
mat.diffuse_color = (1.0,0.2,0.2,1)
missile_1 = Missile(0.8, 5.0, (0.0, 0.0, 2.0), material=mat)
